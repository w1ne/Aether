...
//! Aether Daemon - The gRPC server for Aether Debugger.
                ...
                if let Ok(cmd) = cmd_rx.recv() {
                    match cmd {
                        ...
                        DebugCommand::GetTasks => {
                            let tasks = vec![aether_core::TaskInfo {
                                name: "Idle".to_string(),
                                priority: 0,
                        ...
                        DebugCommand::StartFlashing(_) => {
                            ...
                            let _ =
                                event_tx.send(DebugEvent::FlashStatus("Erasing...".to_string()));
                        ...
                        DebugCommand::WatchVariable(name) => {
                            let _ = event_tx.send(DebugEvent::VariableResolved(
        ...
        if args.chip == "auto" {
            ...
            if !probes.is_empty() && args.probe_index < probes.len() {
                ...
                match probe_manager.connect(
                    ...
                    Err(e) => {
                        error!("Initial attachment failed: {e}. Starting in disconnected mode.");
                    }
                }
            }
        }
